...
cur.execute(
    """CREATE TABLE IF NOT EXISTS trd_chats (chat_id INTEGER,
                                                     chat_title,
                                                     chat_points INTEGER,
                                                     chat_lang,
                                                     is_banned INTEGER,
...
cur.execute(
    """CREATE TABLE IF NOT EXISTS flood_ctl (chat_id INTEGER,
                                                     user_id INTEGER,
...
def valid_point(chat_id, user_id, timestamp):
    usr = cur.execute(
        "SELECT last_timestamp from flood_ctl WHERE chat_id = ? AND user_id = ?",
        (chat_id, user_id),
    )
    res = cur.fetchone()
    if res:
        cur.execute(
            "UPDATE flood_ctl SET last_timestamp = ? WHERE chat_id = ? AND user_id = ?",
            (timestamp, chat_id, user_id),
        )
        return res[0] <= timestamp - 2
    else:
        cur.execute(
            "INSERT INTO flood_ctl (chat_id, user_id, last_timestamp) VALUES (?,?,?)",
...
def get_configs(chat_id: int):
    ...
    return dict(
        ...
        link_optin=res[2],
        link=res[3],
        notifications_optin=res[4],
    )


def change_configs(chat_id: int, config_name: str, value: str or int):
    cur.execute(
        f"UPDATE trd_chats SET {config_name} = ? WHERE chat_id = ?", (value, chat_id)
    )
    con.commit()
